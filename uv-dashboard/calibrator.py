...
def load_enrollment_orders(filepath):
    ...
    for row in range(2, ws.max_row + 1):
        ...
        grade = str(g(23) or '')
        subject = str(g(24) or '')
        if 'K9' not in biz or '暑' not in season or '初一' not in grade:
            continue
...
def calibrate(enrollment_file, old_ledger_file, fall_orders_file=None):
    ...
    old_manual = load_old_ledger_manual_fields(old_ledger_file)
    old_subject = load_old_ledger_subject_data(old_ledger_file)
...
def generate_calibration_report(result, output_path):
    ...
    ws1.cell(row=1, column=1, value='UV台帳 校准报告').font = Font(name='微软雅黑', size=14, bold=True)
    ws1.cell(row=2, column=1, value=f"校准日期: {stats['calibration_date']}  暑/初一/三科").font = Font(name='微软雅黑', size=9, color='666666')

    r = 4
    # 基本信息
    items = [
        ('学员总数', stats['total_students']),
        ('在读学员', stats['active_students']),
        ('新增学员', stats['new_students']),
        ('新增退费学员', stats['new_refund_students']),
        ('新增退费单科数', stats['new_refund_details']),
        ('存量退费学员', stats['all_refund_students'] - stats['new_refund_students']),
        ('存量退费单科数', stats['all_refund_details'] - stats['new_refund_details']),
        ('变动学员数', stats['changes_count']),
    ]
    ws1.cell(row=r, column=1, value='基本信息').font = section_font
    ws1.cell(row=r, column=1).fill = grey_fill
    ws1.merge_cells(start_row=r, start_column=1, end_row=r, end_column=2)
    r += 1
    for label, val in items:
        ws1.cell(row=r, column=1, value=label).font = normal_font
        ws1.cell(row=r, column=2, value=val).font = bold_font
        ws1.cell(row=r, column=2).alignment = align_center
        r += 1

    r += 1
    # 变动类型统计
    ws1.cell(row=r, column=1, value='变动类型统计').font = section_font
    ws1.cell(row=r, column=1).fill = grey_fill
    ...
    if breakdown:
        ws1.cell(row=r, column=1, value='变动类型').font = bold_font
        ws1.cell(row=r, column=2, value='数量').font = bold_font
        ws1.cell(row=r, column=1).fill = grey_fill
        ws1.cell(row=r, column=2).fill = grey_fill
        r += 1
        for ctype, cnt in sorted(breakdown.items(), key=lambda x: -x[1]):
            ws1.cell(row=r, column=1, value=ctype).font = normal_font
            ws1.cell(row=r, column=2, value=cnt).font = normal_font
            ws1.cell(row=r, column=2).alignment = align_center
            r += 1
    else:
        ws1.cell(row=r, column=1, value='无变动').font = normal_font
        r += 1

    # 退费比例
    r += 1
    ws1.cell(row=r, column=1, value='退费比例（新增退费）').font = section_font
    ws1.cell(row=r, column=1).fill = grey_fill
    ws1.merge_cells(start_row=r, start_column=1, end_row=r, end_column=2)
    r += 1
    ...
    ws1.cell(row=r, column=1, value='新增课前退单科数').font = normal_font
    ws1.cell(row=r, column=2, value=before_count).font = normal_font
    r += 1
    ws1.cell(row=r, column=1, value='新增课后退单科数').font = normal_font
    ws1.cell(row=r, column=2, value=after_count).font = normal_font
    r += 1
    if total_refund > 0:
        ws1.cell(row=r, column=1, value='课前退占比').font = normal_font
        ws1.cell(row=r, column=2, value=f'{before_count/total_refund*100:.0f}%').font = normal_font
    else:
        ws1.cell(row=r, column=1, value='无新增退费').font = normal_font

    r += 2
    ws1.cell(row=r, column=1, value='重点关注').font = section_font
    ws1.cell(row=r, column=1).fill = grey_fill
    ...
    for sc in student_changes:
        if sc['change_type'] == '新增学员':
            highlight_items.append(f"• 新增: {sc['name']}({sc['uid']}) {sc['teaching_point']} — {sc['summary']}")
        elif '新增退费' in sc.get('summary', ''):
            highlight_items.append(f"• 退费: {sc['name']}({sc['uid']}) — {sc['summary']}")

    if highlight_items:
        for h in highlight_items[:20]:
            ws1.cell(row=r, column=1, value=h).font = normal_font
            ws1.merge_cells(start_row=r, start_column=1, end_row=r, end_column=4)
            r += 1
    else:
        ws1.cell(row=r, column=1, value='• 无异常变动').font = normal_font
